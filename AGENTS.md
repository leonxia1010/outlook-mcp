...
This is a modular MCP (Model Context Protocol) server that provides Codex with access to Microsoft 365 services:
...
- **For Codex Desktop config**: Use `OUTLOOK_CLIENT_ID` and `OUTLOOK_CLIENT_SECRET`
...
Token-storage tests cover warning text and token-file mtime reload behavior. Treat failures in
`test/auth/token-storage.test.js` as real regressions unless intentionally changing auth logging
or token cache reload semantics.
